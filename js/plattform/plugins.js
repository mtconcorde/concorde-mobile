...
function formPopulateLocations(locationfieldID,locations){
...
//     formPopulateField(field, options, field.getAttribute('data-placeholder'));
	var field = $(locationfieldID);
	field.html('<option value="">Select a campus...</option>');

	for(var l in locations) {
		var location = locations[l];
		var option = '<option value="' + location.velocify_id + '">' + location.city + '</option>';
		field.append(option);
	}

}
function formPopulatePrograms(programfieldID,programs){
...
// 	}
	var field = $(programfieldID);
	field.html('<option value="">Select a program...</option>');

	for(var l in programs) {
		var program = programs[l];
		var option = '<option value="' + program.velocify_program_id + '">' + program.program + '</option>';
		field.append(option);
	}
...
jQuery().ready(function () {
	jQuery('#zip').change(function() {
        lookupZip(jQuery(this),jQuery('#city'),jQuery('#state'));
    });

    //prevent sticky header from covering inputs on ppc mobile
	jQuery('.node-type-basic-landing #submit').click(function(){
		jQuery('.node-type-basic-landing').addClass('no-sticky');
    });

    var campusItems = jQuery('body.pf-template .program-campuses li');
    var programItems = jQuery('body.pf-template .campus-programs li');
    if (campusItems.length == 1) {
	    jQuery('.program-campuses').addClass('one-column');
    } else if (programItems.length == 1) {
	    jQuery('.campus-programs').addClass('one-column');
    }
});

var lookupZip = function( $zip, $city, $state ) {
    var fail = function() {
        $city.val(''); $state.val('');
    }
    var z = $zip.val();
    ...
    $state.val('');
     $.ajax({
        url: 'http://plattformstuff.com/zip.php',
        jsonp: 'callback',
        dataType: 'jsonp',
        data: {
            zip: z
...
(function(e){
		...
		jQuery.each(fields,function(){
			switch(this.tagName){
				case 'SELECT':
					...
					break;
				case 'INPUT':
					if (!Modernizr.input.placeholder && this.tagName == 'INPUT' && this.getAttribute('type') !== 'checkbox'){
    ...
    e.fn.enterpriseFormInit = function(settings) {
		...
		if(isAndroid == false){
			...
			jQuery('#enterpriseform input[data-validate~="phoneUS"], #phone1').attr('maxlength','10');
			jQuery('#enterpriseform input[data-validate~="zipUS"]').attr('maxlength','5');
		}
        return this;
    };
}(jQuery));